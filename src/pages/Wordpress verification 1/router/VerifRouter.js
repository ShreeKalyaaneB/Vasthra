...
VerifRouter.get("/verif-veriflist", async function (req, res) {
	try {
	  const verifListData = await req.db.collection("sarees").find().toArray();
...
VerifRouter.get('/verify-form/:id', async function (req, res) {
	try {
	  ...
	  const property = await req.db.collection("sarees").findOne({ _id: ObjectId(propertyId) });
	  if (property) {
...
VerifRouter.post('/get-verif-verifform', async function (req, res) {
	...
	try {
		...
		DataRequest.push(
			req.db.collection(sarees).aggregate([
				{
...
VerifRouter.get('/verif-verifform', async function (req, res) {
	try {
		const updatedProperty = await req.db.db("vasthra").collection("sarees").find().toArray();
		const data = {
...
VerifRouter.post('/verif-verifform', async function (req, res) {
	try {
		...
		var Fabric = req.body.txtFabric;
		var Material = req.body.txtMaterial;
		var Zari= req.body.txtZari;
		var Manufacturer=req.body.txtManufacturer;
		var SilkHallmark= req.body.txtSilkHallmark;
		var Colour = req.body.txtColour;
		var Weight = req.body.txtWeight;
		var Length= req.body.txtLength;
		// var Age = req.body.txtAge;
		// var Sex = req.body.lstSex;
		// var DiseaseTolerance = req.body.txtDiseaseTolerance;
		...
		if (WriteStatus.txstatus) {
			...
			const updateResult = await req.db.collection("sarees").updateOne(
				{ _id: ObjectId(PostID) },
				{ $set: { "verifyStatus": "verified" } }
			);

			if (updateResult.modifiedCount > 0) {
				const updatedProperty = await req.db.collection("sarees").findOne({ _id: ObjectId(PostID) });
